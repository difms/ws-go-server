...
func addPresence(channelName, userID string) error {
	...
	err := redisPool.SAdd(presenceKey, userID).Err()
	if err != nil {
		return err
	}

	// Отправляем оповещение о входе пользователя
	notification := Notification{
		UserID:  0, // Можно использовать 0 для системных сообщений
		Data:    map[string]interface{}{"event": "user_joined", "user_id": userID},
		Channel: channelName,
	}
	notificationJSON, _ := json.Marshal(notification)
	redisPool.Publish(channelName, notificationJSON) // Публикуем в канал
...
func removePresence(channelName, userID string) error {
	presenceKey := fmt.Sprintf("%s%s", presencePrefix, channelName)
	err := redisPool.SRem(presenceKey, userID).Err()
	if err != nil {
		return err
	}

	// Отправляем оповещение о выходе пользователя
	notification := Notification{
		UserID:  0, // Можно использовать 0 для системных сообщений
		Data:    map[string]interface{}{"event": "user_left", "user_id": userID},
		Channel: channelName,
	}
	notificationJSON, _ := json.Marshal(notification)
	redisPool.Publish(channelName, notificationJSON) // Публикуем в канал
